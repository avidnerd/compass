---
name: Compass
description: A one-bit desktop printed in two plates — black, paper, and one stamp ink that only ever marks verified evidence.
colors:
  ink: "#000000"
  paper: "#ffffff"
  stamp: "#d8202a"
...
Compass is drawn the way a one-bit workstation was drawn: one ink on one paper, with every intermediate value achieved by knocking pixels out of a tile rather than by mixing. The tokens are literally named `--ink` and `--paper`, and that naming is the whole thesis — this is not a palette that happens to be monochrome, it is a printing process. Grays do not exist; ordered dithers stand in for all of them. The desktop ground itself is a 25% dither, so every window sits on visible tooth rather than on flat white.

There is a second plate, and it is the only colour in the system: **Stamp Vermilion**, `--stamp`. It behaves like a spot ink on a two-colour press run — it never blends, never tints, and never decorates. Its single job is marking evidence a machine verified, which is the one thing this product exists to prove. A student can scan any screen and know that every vermilion mark is backed by a real file, commit, email or calendar block Compass observed for itself. That restraint is what makes the colour mean anything.
...
**Key Characteristics:**
- Two plates: black on paper, plus one stamp ink that only ever marks verified evidence.
...
### Secondary
- **Stamp Vermilion** (`#d8202a`): the second plate, reserved entirely for verified evidence — the box on a verified log row, the box on a verified subgoal, and the `connected` connector badge. 5.05:1 on paper and 4.16:1 on ink, so it holds on both grounds. It appears as a hairline border and a glyph, never as a filled field: a stamp is pressed onto a record, it does not flood it. Inside inverted chrome it steps aside for `currentColor`, because a red mark on a black bar is unreadable.
...
- **12% Speckle** (`--d12`, 4px tile): the lightest tint. Row striping in tables, list-item and player-card hover, hero grounds, the soft-glow habitat, the `unclear` attention class.
- **25% Ground** (`--d25`, 2px tile): the desktop itself, the scrollbar track, and the default button hover.
...
- **Forward Hatch** (`--hatch`, 4px diagonal): the window resize grip, the danger button's warning band, the ember habitat and ember avatar.
...
**The Two-Plate Rule.** This is a two-colour press run, not a palette. Plate one is `#000000` on `#ffffff`. Plate two is **Stamp Vermilion** (`#d8202a`) and it has exactly one job: marking evidence a machine verified. Nothing else may take it — not a brand accent, not danger, not a chart series, not the active nav item. If a design needs another value, it needs another pattern. The ink means "this was checked", and it stops meaning that the moment it is used for anything else.

**The Earned Colour Rule.** Colour on this surface is a claim about truth, so it is never decorative. A student can scan any screen and know that every vermilion mark corresponds to a real file, commit, email or calendar block Compass actually observed. Steps the student merely asserts stay one-bit, and errors stay one-bit too — they use the back-hatch fill and the solid `!` box, which is how this system has always shown trouble.
...
- **Body** (Geneva 400, 13px, line-height 1.55, max-width 68ch): all running prose. Hero paragraphs cap at 58ch.
...
- **Caption** (Geneva 400, 11px): the tertiary step — `.small`, `h3`, inline `code`. Below this, type switches to Silkscreen and uppercase.
...
- **600px** — the phone pass: player grid to one column, hero and co-op launcher stack, window body padding tightens to 10px/11px.
...
### Shadow Vocabulary
- **Dithered plate** (`::before` at `top:3px; left:3px; right:-3px; bottom:-3px`, `background-image: var(--d50)`, `background-size: 2px 2px`, `z-index: -1`): the standard lift for any floating surface — windows, cards, toasts, player cards and heroes.
...
**The One-Pixel Chrome Rule.** Structural borders are `1px solid var(--ink)`. 2px is the only escalation and it means urgency: an error note, a live monitor that is actively sharing, a danger button. Dotted 1px is the de-escalation, used for divisions inside a surface — list rows, log rows, rules, and advisory boxes.
...
- **Focus:** a 2px dashed ink outline offset 2px. Inside inverted chrome (title bars, menu bar, active desk icons, an active button) the ring flips to paper, or it would draw black on black.
